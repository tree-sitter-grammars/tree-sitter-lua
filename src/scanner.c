...
bool tree_sitter_lua_external_scanner_scan(void *payload, TSLexer *lexer,
    ...
    if (valid_symbols[MULTILINE_STRING_END] && scan_block_end(lexer)) {
        reset_state();
        lexer->result_symbol = MULTILINE_STRING_END;
        return true;
    }

    if (valid_symbols[MULTILINE_STRING_CONTENT] && scan_block_content(lexer)) {
    ...
    if (valid_symbols[MULTILINE_STRING_START] && scan_block_start(lexer)) {
